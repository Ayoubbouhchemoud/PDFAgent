"""
pdf_to_txt.py — PDF to plain-text converter.

Reuses the same pdfplumber-based structural extraction as pdf_to_docx.py
and outputs every logical block as clean UTF-8 text:

  Headings            → text followed by an underline of dashes
  Body paragraphs     → plain text with blank lines between them
  Bullet lists        → • item text, one per line
  Code/syntax boxes   → text indented with 4 spaces
  Side-by-side pairs  → label, then each box indented
  Tabular data        → tab-separated columns
  Page breaks         → a ruler (──── … ────) between pages
  Images              → skipped
"""
...
def _process_page(out: list[str], page, is_first: bool) -> None:
    ...
    def write_table(words_list: list[dict], col_starts: list[float]) -> None:
        ...
        _ensure_blank(out)
        for line in lines:
            out.append("\t".join(row_cells(line)))
        out.append("")
    ...
    for (y, kind, data) in events:
        ...
        if kind == "header":
            ...
            if ll or rl:
                _ensure_blank(out)
                out.append(f"{ll}    {rl}".strip())
                out.append("")

            for label, words in ((ll, l_words), (rl, r_words)):
                if not words:
                    continue
                if label:
                    out.append(f"  [{label}]")
...
def convert(pdf_path: str | Path) -> str:
    """Convert a PDF to plain text and return the string."""
...
if __name__ == "__main__":
    ...
    if len(_sys.argv) < 2:
        print("Usage: python pdf_to_txt.py <input.pdf> [output.txt]", file=_sys.stderr)
        _sys.exit(1)

    src = Path(_sys.argv[1])
    dst = Path(_sys.argv[2]) if len(_sys.argv) > 2 else src.with_suffix(".txt")
